...
public class CowBarnState implements IState {
    ...
    private int originalStallIndexOfPregnant = 5;
    private int stallIndexOfUnbornBaby = 6;
        ...
        for (int i = 0; i < 12; i++) {
            if (fodderToCowHashMap.get(fodderDisplayerTileArray[i]) != null) {
                ...
                if ( (!fodderDisplayerTileArray[i].isActivated()) &&
                        ...
                        //@@@
                        || (tempCow.getCowState() == Cow.CowState.BABY) || (tempCow.getCowState() == Cow.CowState.CALF)
                        //@@@
                        ) ) {
